...
def run(args):
    ...
    for file in sorted(dir):
        ...
        dir_name = os.path.basename(args.probs_map_path).split('2s')[0] + 'l{}'.format(8)
        tissue = np.load(os.path.join(os.path.dirname(args.probs_map_path), dir_name, file))
        tissue_shape = tuple([int(i / 2**level) for i in slide.level_dimensions[0]])
...
def main():
    ...
    args = parser.parse_args([
        ...
        "/home/ps/hhy/slfcd/save_train/train_ncrf/resnet18_base.ckpt",
        "/home/ps/hhy/slfcd/camelyon16/configs/cnn_ncrf.json",
        '/media/ps/passport2/hhy/camelyon16/test/dens_map_ncrf_2s_t0.5_l6'])
    args.GPU = "2"
