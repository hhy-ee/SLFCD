...
from camelyon16.data.prob_producer_base1 import WSIPatchDataset  # noqa
...
def get_probs_map(model, dataloader):
    probs_map = np.zeros((dataloader.dataset.pad_x_len, dataloader.dataset.pad_y_len))
    ...
    with torch.no_grad():
        for (data, rect, shape) in dataloader:
            ...
            probs = output['out'][:, :].sigmoid().cpu().data.numpy().transpose((0,2,3,1))
            probs =  np.array([patch * dataloader.dataset.WINDOW_SPLINE_2D for patch in probs])
        ...
        probs_map = probs_map / (dataloader.dataset._subdivisions ** 2)
        aug = dataloader.dataset._aug
        probs_map = probs_map[aug:-aug, aug:-aug]
    return probs_map


def make_dataloader(args, cnn, slide, flip='NONE', rotate='NONE'):
    batch_size = cnn['batch_inf_size']
    num_workers = args.num_workers

    dataloader = DataLoader(
        WSIPatchDataset(slide, args.level, args.subdivisions,
    ...
    for file in dir:
        ...
        if not args.eight_avg:
            dataloader = make_dataloader(
                args, cnn, slide, flip='NONE', rotate='NONE')
