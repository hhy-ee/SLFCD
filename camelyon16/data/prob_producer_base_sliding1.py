...
class WSIPatchDataset(Dataset):
    ...
    def __init__(self, slide, tissue, level, overlap, image_size=256,
        ...
        self._tissue = np.asarray(PIL.Image.fromarray(tissue.transpose()).resize(slide.level_dimensions[level])).transpose()
    ...
    def __getitem__(self, idx):
        ...
        if x_size !=self._image_size or y_size !=self._image_size:
            ...
            img.paste(img_temp, (0, 0, x_size, y_size))
        ...
        if self._tissue[x: x + x_size, y:y + y_size].any():
            self._keep = True
        else:
            self._keep = False

        return (img, (x, y, x + x_size, y + y_size), (x_size, y_size))
