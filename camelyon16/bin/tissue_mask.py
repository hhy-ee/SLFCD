...
parser.add_argument('tumor_path', default=None, metavar='TUMOR_PATH', type=str,
                    help='Path to the output tumor npy mask file')
...
def run(args):
    ...
    for file in dir:
        ...
        if 'tumor' in file:
            tissue_mask = tissue_mask | np.load(os.path.join(args.tumor_path, file.split('.')[0] + '.npy'))
        np.save(os.path.join(args.tissue_path, file.split('.')[0] + '.npy'), tissue_mask)

def main():
    args = parser.parse_args([
        "/media/ps/passport2/hhy/camelyon16/training/normal",
