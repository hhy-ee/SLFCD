...
def parse_args():
    ...
    args = parser.parse_args(['/media/ps/passport2/hhy/camelyon16/test/crop_split_2048_sampling_l1/results.json', 
                              ...
                              '/media/ps/passport2/hhy/camelyon16/test/crop_cluster_2048_sampling_l1'])
...
if __name__ == "__main__":
    ...
    for file_name in tqdm(candidates.keys(), total=len(candidates.keys())):
        ...
        size_dens = tuple([int(i / 2**dens_level) for i in slide.level_dimensions[0]])
        size_output = tuple([int(i / 2**output_level) for i in slide.level_dimensions[0]])
        dens_map_down = cv2.resize(dens_map.transpose(), size_dens, interpolation=cv2.INTER_CUBIC).transpose() / 255
        dens_map_up = cv2.resize(dens_map.transpose(), size_output, interpolation=cv2.INTER_CUBIC).transpose() / 255
        ...
        for box in bbox_info:
            ...
            score = dens_map_down[left:right, top:bot].mean()
        ...
        for i, cluster in enumerate(cluster_boxes_dict):
            ...
            dens_patch = dens_map_up[clu_box[0]: clu_box[0]+clu_box[2], clu_box[1]: clu_box[1]+clu_box[3]]
            extractor = extractor_features(dens_patch, slide_path)
